...
public class StudentDaoImpl implements DaoService<Student> {
    ...
    public int updateData(Student object) {
        ...
        try {
            Connection connection = DBUtil.createMySQLConnection();
            String query
                    = "UPDATE student SET first_name = ?, last_name = ?, birth_date = ?, department_id = ? WHERE id = ?";
            try (PreparedStatement ps = connection.prepareStatement(query)) {
                ps.setString(5, object.getId());
                ps.setString(1, object.getFirstName());
                ps.setString(2, object.getLastName());
                ps.setDate(3, new java.sql.Date(object.getBirthDate().getTime()));
                ps.setInt(4, object.getDepartment().getId());
                if (ps.executeUpdate() != 0) {
                    connection.commit();
                    result = 1;
                }
